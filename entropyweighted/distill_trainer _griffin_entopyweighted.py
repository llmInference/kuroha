...
class DistillTrainer(Trainer):
    def __init__(self, teacher_model, *args, **kwargs):
        ...
        # GRIFFIN inspired parameter for token alignment
        self.topk_alignment = getattr(args, "topk_train", 3) # Use topk_train from args, default to 3
        
        # ==================== START: EA-KD Paper Integration ====================
        # Add hyperparameter for EA-KD entropy calculation temperature (T')
        self.ea_kd_entropy_temp = getattr(args, "ea_kd_entropy_temp", 3.0)
        # ===================== END: EA-KD Paper Integration =====================
    ...
    def online_training_step(self, model, inputs):
        ...
        # The generated_ids from the generator are the "ground truth" sequence verified by the teacher model
        token_ids = torch.cat([input_ids, output.generated_ids], dim=-1)
        
        # We don't need wrong_token_ids for the GRIFFIN-style loss mask, but we keep the data collection logic
        wrong_token_ids = [
        ...
        if "dataset" in inputs:
            ...
            # This buffer logic seems specific to an experiment, keeping it as is.
        ...
        if len(self.buffer) >= self.online_update_interval:
            ...
            # ==================== START: Hybrid GRIFFIN + EA-KD Logic ====================
            # The logits at position t are used to predict the token at t+1.
            # So, we match student_logits[:, :-1] with labels input_ids[:, 1:]
            student_logits_for_pred = student_logits[..., :-1, :].float()
            teacher_logits_for_pred = teacher_logits[..., :-1, :].float()

            # The ground-truth tokens (labels) are the next tokens in the sequence.
            labels = input_ids[..., 1:].clone()

            # We also ignore loss on padding tokens.
            padding_mask = (labels == self.tokenizer.pad_token_id)

            # --- Step 1: GRIFFIN Filtering ---
            # Create a mask for severely misaligned tokens, where loss weight will be zero.
            _, topk_indices = torch.topk(student_logits_for_pred, k=self.topk_alignment, dim=-1)
            labels_expanded = labels.unsqueeze(-1).expand_as(topk_indices)
            matches = torch.any(topk_indices == labels_expanded, dim=-1)
            # `alignment_mask` is True for tokens to be IGNORED (i.e., where there is no match).
            alignment_mask = ~matches

            # --- Step 2: EA-KD Reweighting ---
            # Calculate EA-KD weights for ALL non-padded tokens.
            student_entropy = self._calculate_entropy(student_logits_for_pred, self.ea_kd_entropy_temp)
            teacher_entropy = self._calculate_entropy(teacher_logits_for_pred, self.ea_kd_entropy_temp)
            device = student_logits_for_pred.device
            h_ub = torch.log(torch.tensor(self.model.config.vocab_size, device=device, dtype=torch.float32))
            w_base = teacher_entropy
            w_interact = (teacher_entropy * student_entropy) / (h_ub ** 2)
            w_ea = (w_base + w_interact) / 2.0
            
            # --- Step 3: Combination ---
            # Start with the calculated EA-KD weights.
            final_weights = w_ea
            # Apply the GRIFFIN filter: set the weights of misaligned tokens to 0.0.
            final_weights.masked_fill_(alignment_mask, 0.0)
            
            # The loss function will now use these combined weights.
            loss = self.soft_cross_entropy(
                student_logits_for_pred, 
                teacher_logits_for_pred, 
                padding_mask,
                weights=final_weights
            )
            # ===================== END: Hybrid GRIFFIN + EA-KD Logic =====================
    ...
    # ==================== START: New Helper Function for EA-KD ====================
    def _calculate_entropy(self, logits, temperature=1.0):
        """
        Calculates the Shannon entropy for a given batch of logits,
        using temperature T' as described in the EA-KD paper.
        """
        probs = torch.softmax(logits / temperature, dim=-1)
        log_probs = torch.log_softmax(logits / temperature, dim=-1)
        entropy = -torch.sum(probs * log_probs, dim=-1)
        return entropy
    # ===================== END: New Helper Function for EA-KD =====================
    ...
    # ==================== START: Modified Loss Function for Weighted Loss ====================
    def soft_cross_entropy(self, predicts, targets, padding_mask, weights=None):
        """
        Calculates soft cross-entropy. If `weights` are provided, it computes a weighted loss.
        """
        predict_log_prob = torch.nn.functional.log_softmax(predicts, dim=-1)
        targets_prob = torch.nn.functional.softmax(targets, dim=-1)
        
        # Calculate loss for each token individually
        per_token_loss = -torch.sum(targets_prob * predict_log_prob, dim=-1)
        # Apply EA-KD / Hybrid weights if provided
        if weights is not None:
            per_token_loss = per_token_loss * weights
        # Always apply the padding mask to ensure padded tokens have zero loss
        per_token_loss.masked_fill_(padding_mask, 0.0)
        
        # Normalize the loss
        if weights is not None:
            # When weights are provided, normalize by the sum of effective weights
            # (excluding padded tokens and tokens with zero weight)
            effective_weights = weights.clone()
            effective_weights.masked_fill_(padding_mask, 0.0)
            normalization = effective_weights.sum() + 1e-8
        else:
            # When no weights, normalize by the number of active (non-padded) tokens
            normalization = (~padding_mask).sum() + 1e-8
        
        total_loss = per_token_loss.sum()
        mean_loss = total_loss / normalization
        return mean_loss
    # ===================== END: Modified Loss Function for Weighted Loss =====================
    ...
    def get_kl(self, predicts, targets, padding_mask):
        ...
        mean_output = output.sum() / ((~padding_mask).sum() + 1e-8) # Add epsilon for stability
        return mean_output
