...
class DistillTrainer(Trainer):
    ...
    def online_training_step(self, model, inputs):
        ...
        if len(self.buffer) >= self.online_update_interval:
            ...
            # --- START OF GRIFFIN-STYLE MASK MODIFICATION ---
            ...
            # Get the student's top-k predictions.
            _, topk_indices = torch.topk(student_logits_for_pred, k=self.topk_alignment, dim=-1)

            # Expand labels to match the shape of topk_indices for broadcasting.
            labels_expanded = labels.unsqueeze(-1).expand_as(topk_indices)

            # `matches` is True where the correct label is within the student's top-k predictions.
            matches = torch.any(topk_indices == labels_expanded, dim=-1)

            # The loss mask should be True where we want to IGNORE the loss.
            # We ignore the loss if there is NO match (i.e., severe misalignment).
            alignment_mask = ~matches

            # We also ignore loss on padding tokens.
            padding_mask = (labels == self.tokenizer.pad_token_id)

            # The final mask combines both conditions. We ignore a token if it's padding OR it's misaligned.
            # A `True` value in this mask means the loss for that token will be zeroed out.
            final_mask = alignment_mask | padding_mask
            
            # The soft_cross_entropy function expects a mask where True means "to pad" or "to ignore".
            # Our `final_mask` is already in this format.
            loss = self.soft_cross_entropy(
                student_logits_for_pred, 
                teacher_logits_for_pred, 
                final_mask
            )
            # --- END OF GRIFFIN-STYLE MASK MODIFICATION ---
    ...
    def soft_cross_entropy(self, predicts, targets, padding_mask):
        ...
        mean_entropy = entropy.sum() / ((~padding_mask).sum() + 1e-8) # Add epsilon for stability
        return mean_entropy
