...
from sglang.srt.speculative.online_distillation import create_distillation_manager
...
class EAGLEWorker(TpModelWorker):
    ...
    def __init__(
        ...
        self.distillation_manager = create_distillation_manager(
            self.server_args, self.draft_model_runner.model, self.target_worker.model_runner.model, self.target_worker.tokenizer
        )
    ...
    def verify(self, batch: ScheduleBatch, spec_info: EagleVerifyInput):
        ...
        # 收集蒸馏样本（如果启用了在线蒸馏）
        if hasattr(self, 'distillation_manager') and self.distillation_manager is not None:
            self.distillation_manager.collect_sample(batch, res)
