...
class ServerArgs:
    ...
    # Online distillation for speculative decoding
    online_distillation_enable: bool = False
    online_distillation_buffer_size: int = 1000
    online_distillation_update_interval: int = 10
    online_distillation_learning_rate: float = 1e-5
    online_distillation_max_grad_norm: float = 1.0
    online_distillation_temperature: float = 1.0
    online_distillation_kl_weight: float = 1.0
    online_distillation_focal_gamma: float = 0.0
    online_distillation_topk_alignment: int = 3
    ...
    def add_cli_args(parser: argparse.ArgumentParser):
        ...
        # Online distillation for speculative decoding
        parser.add_argument(
            "--online-distillation-enable",
            action="store_true",
            help="Enable online distillation for speculative decoding to align draft and target models.",
        )
        parser.add_argument(
            "--online-distillation-buffer-size",
            type=int,
            default=ServerArgs.online_distillation_buffer_size,
            help="Buffer size for collecting distillation samples.",
        )
        parser.add_argument(
            "--online-distillation-update-interval",
            type=int,
            default=ServerArgs.online_distillation_update_interval,
            help="Number of samples to collect before updating the draft model.",
        )
        parser.add_argument(
            "--online-distillation-learning-rate",
            type=float,
            default=ServerArgs.online_distillation_learning_rate,
            help="Learning rate for online distillation.",
        )
        parser.add_argument(
            "--online-distillation-max-grad-norm",
            type=float,
            default=ServerArgs.online_distillation_max_grad_norm,
            help="Maximum gradient norm for clipping during distillation.",
        )
        parser.add_argument(
            "--online-distillation-temperature",
            type=float,
            default=ServerArgs.online_distillation_temperature,
            help="Temperature for distillation loss computation.",
        )
        parser.add_argument(
            "--online-distillation-kl-weight",
            type=float,
            default=ServerArgs.online_distillation_kl_weight,
            help="Weight for KL divergence loss in distillation.",
        )
        parser.add_argument(
            "--online-distillation-focal-gamma",
            type=float,
            default=ServerArgs.online_distillation_focal_gamma,
            help="Gamma parameter for focal loss in distillation.",
        )
        parser.add_argument(
            "--online-distillation-topk-alignment",
            type=int,
            default=ServerArgs.online_distillation_topk_alignment,
            help="Top-k value for alignment masking in distillation.",
        )
